...
float2 AdjustedDepth(half2 uvs, half4 additionalData) {
    ...
    // TODO: Changing the usage of UNITY_REVERSED_Z this way to fix testing, but I'm not sure ther original code correct anyway.
...
float WaterTextureDepth(float3 posWS) {
    return (1 - SAMPLE_TEXTURE2D_LOD(
        _WaterDepthMap, 
        sampler_WaterDepthMap_linear_clamp, 
        posWS.xz * 0.002 + 0.5, 1).r) * (_MaxDepth + _VeraslWater_DepthCamParams.x) - _VeraslWater_DepthCamParams.x;
}
...
half3 Refraction(half2 distortion, half depth, real depthMulti) {
    half3 output = SAMPLE_TEXTURE2D_LOD(
        _CameraOpaqueTexture, 
        sampler_cameraOpaqueTexture_linear_clamp,
        distortion,
        depth * 0.25
    ).rgb;
...
half4 AdditionalData(float3 positionWS, WaveStruct wave) {
    ...
    data.y = length(GetCameraPositionWS().xyz - positionWS); // local position in camera sapace
...
WaterVertexOutput WaveVertexOperations(WaterVertexOutput input) {
    ...
    input.uv.xy = input.posWS.xz * 0.1h - time.xx * 0.1h + (input.fogFactorNoise.y * 0.2);
...
half4 WaterFragment(WaterVertexOutput IN) : SV_Target {    
    ...
    half2 detailBump = (detailBump1 + detailBump2 * 0.5) * saturate(depth.x * 0.25 + .25);
    ...
    distortion = screenUV.xy + distortion; // * clamp(depht.x, 0, 5);
    float d = depth.x;
    ...
    half3 GI = SampleSH(IN.normal); // Global Illumination
